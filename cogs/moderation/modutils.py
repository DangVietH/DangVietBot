...
class ModUtils(commands.Cog):
    ...
    async def add_to_config(self, guild):
        ...
        if results is None:
            await cursors.insert_one({"guild": guild.id, "modlog": 0, "role": 0,
                                      "automod": {"spam": False, "invite": False, "links": False, "mass_mention": False,
                                                  "raid": False, "caps": False}})
    ...
    @modConfig.command(help="Set up channel")
    @commands.has_permissions(manage_channels=True)
    async def modlog(self, ctx, channel: discord.TextChannel):
        await self.add_to_config(ctx.guild)
        await cursors.update_one({"guild": ctx.guild.id}, {"$set": {"modlog": channel.id}})
