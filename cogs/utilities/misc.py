...
class Misc(commands.Cog):
    ...
    async def covid(self, ctx, *, country="vietnam"):
        ...
        url = requests.get(f"https://disease.sh/v3/covid-19/countries/{country}")
        stats = url.json()
        embed = discord.Embed(title=f"Covid Information in {stats['country']}", description=f"Last updated: <t:{int(stats['updated'] / 1000)}:R>")
